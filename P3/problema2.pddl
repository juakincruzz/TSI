...
; Problema: formar una Comunidad reducida y destruir el Anillo Unico.

(define (problem tierra-media-problema2)
  (:domain tierra-media-ej2)
  ...
  (:init
    ...
    ; Capacidades instanciadas para la accion ExtraerRecurso.
    ...
    (disponible Mago2)
    (disponible Enano1)
    ; Enano2 sigue ocupado en una mision diplomatica y no esta disponible.
    ...
    ; Objetos de la mision del Anillo.
    (objetoEn Anillo Rivendell)
    (objetoEn Espada Lothlorien)
    (objetoEsAnillo Anillo)
    (lugarDestruccion Orodruin)

    ; Inicialmente solo los Hobbits pueden recoger el Anillo. Despues de
    ; recogerlo, el mismo portador queda habilitado para recoger la Espada y
    ; el ChalecoMithril.
    (puedeRecogerObjeto Hobbit1 Anillo)
    (puedeRecogerObjeto Hobbit2 Anillo)
    (puedeRecogerObjeto Hobbit3 Anillo)
    (puedeRecogerObjeto Hobbit4 Anillo)
